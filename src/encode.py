...
def main(args):
    ...
    # 1.4.2 随机张量，对于Qwen2.5-1.5B-Instruct，假设28层都是FFN，每层FFN有gate，up，down3个张量，每个张量都需要A, B
    # 1.4.2 随机张量，对于Qwen2.5-1.5B-Instruct，假设28层都是FFN，每层需要6个LoRA张量，一共需要168个LoRA张量
    ...
    if not os.path.exists(os.path.join(init_adapter_path, "adapter_model.safetensors")):
        # 1.4.1 创建/Qwen2.5-1.5B-Instruct/秩为2,factor为32/adapter_model.safetensors，LoRA张量，一共28*6 = 168个张量
        ...
        # 1.4.5.2 文件内容是28*6 = 168个张量，每个shape分别是[1536, 2] + [2, 8960]，随机
    ...
    # 1.5 设置cot_name = "direct"
    cot_name = "cot" if args.with_cot else "direct"

    # 1.6 获取data_list中单个文件
    # 1.6.1 获取comparison.json
    for filename, fulldata in data_list:
        ...
        # 1.6.2 获取comparison.json中每一个样例id
        # 1.6.2 获取comparison.json中每一个样例augment
        # 1.6.2 获取comparison.json中每一个样例augment的3个文档，也就是一个样例的augment中的一个文档具有三个问答对，一个文档作为adapter_model.safetensors
        # 1.6.2 给样例问题，augment[0]全部数据，也就是文档一，微调前LoRA张量，微调后LoRA张量
        # 1.6.2 给样例问题，augment[1]全部数据，也就是文档二，微调前LoRA张量，微调后LoRA张量
        # 1.6.2 给样例问题，augment[2]全部数据，也就是文档三，微调前LoRA张量，微调后LoRA张量
        for did, data in tqdm(enumerate(fulldata), total=len(fulldata)):
            ...
            for pid in range(len(augment)):
                ...
                if os.path.exists(os.path.join(save_path, "adapter_model.safetensors")): continue
                model = train(data["question"], [augment[pid]], args, model, tokenizer, init_adapter_path, save_path)


# 2. 文档参数化
# 2.1 参数是样例问题，augment[0]全部数据，也就是augment[0]的文档，三个问答对，微调前LoRA张量，微调后LoRA张量
def train(
        question, 
        augments, 
    ...
    # 2.1 对于样例一文档一
    # 2.1.1 可以获取[问题1 + 原文 + 答案1]，[问题1 + 改写 + 答案1]
    # 2.1.1 可以获取[问题2 + 原文 + 答案2]，[问题2 + 改写 + 答案2]
    # 2.1.1 可以获取[问题3 + 答案3]
    # 2.1.1 一共五条prompt
...
if __name__ == "__main__":
    ...
    # 3. LoRA矩阵的shape
    # 3.1 lora_rank就是秩r，也就是gate.shape = [1536, 8960]，那么A.shape = [1536, 2], B.shape = [2, 8960]，这个2就是秩r
    ...
    args.model_name = "/Users/toryn/Remote/PRAG/qwen2.5-1.5b-instruct"
    args.dataset = "2wikimultihopqa"
    args.augment_model = "qwen2.5-1.5b-instruct"
    args.lora_rank = 2
    args.lora_alpha = 32
